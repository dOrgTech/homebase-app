...
const TitleText = styled(Typography)({
  color: ({ theme, customColor }: { theme: Theme; customColor?: string }) =>
    customColor || theme ? theme.palette.text.primary : "#fff",
});
...
export const ResponsiveDialog: React.FC<{
  ...
  onClose: () => void;
  title?: string;
  alignTitle?: "center" | "initial";
  customTitleColor?: string;
}> = ({ open, onClose, customTitleColor, title, children, alignTitle }) => {
  ...
  return isSmall ? (
    ...
    <Dialog open={open} onClose={() => onClose()}>
      <Content container direction={"column"} style={{ gap: 46 }}>
        <Grid item container direction='row' wrap={"nowrap"} justifyContent={"space-between"}>
          <Grid item style={{ width: "100%", textAlign: `${alignTitle ? alignTitle : "initial"}` }}>
            {title && (
