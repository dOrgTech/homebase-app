...
const BottomNavBar: React.FC = ({ children }) => {
  ...
  return (
    <StyledBottomBar container direction={"row"} justify={"space-evenly"} visible={visible}>
      {children}
    </StyledBottomBar>
  )
}
    ...
    <Container container justifyContent="center" style={{ gap: 92 }}>
      {pages.map((page, i) => (
        <PageItem key={`page-${i}`} item alignItems="center">
          <Link to={page.href}>
            <Grid container alignItems="center" justifyContent="center" style={{ gap: 15 }}>
              <Grid item>
              ...
              <Grid item>
                <Typography color={pathId === page.pathId ? "secondary" : "textPrimary"}>{page.name}</Typography>
              </Grid>
            </Grid>
          </Link>
        </PageItem>
